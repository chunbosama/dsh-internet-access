...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var SETUP_URL = "/api/public-access/setup";
		...
		function PublicAccessCard() {
			...
			/** 首次初始化：创建初始用户名+密码账号（仅本机设置页可用）。 */
			var setupUser = function () {
				var u = document.getElementById("dsh-ia-setup-username");
				var p = document.getElementById("dsh-ia-setup-password");
				var c = document.getElementById("dsh-ia-setup-confirm");
				var err = document.getElementById("dsh-ia-setup-error");
				if (err) err.textContent = "";
				var username = u ? u.value.trim() : "";
				var password = p ? p.value : "";
				var confirm = c ? c.value : "";
				if (username === "" || password === "") {
					if (err) err.textContent = "请填写用户名和密码";
					return;
				}
				if (password.length < 8) {
					if (err) err.textContent = "密码至少 8 个字符";
					return;
				}
				if (password !== confirm) {
				...
				var btn = document.getElementById("dsh-ia-setup-submit");
				...
				fetch(SETUP_URL, {
					...
					.then(function (result) {
						if (result.ok) {
							...
							if (err) err.textContent = (result.body && result.body.error && result.body.error.message) || "设置失败";
						}
					})
					.catch(function () {
					...
					.finally(function () {
						if (btn) { btn.disabled = false; btn.textContent = "设置初始账号"; }
					});
			};
				...
				children: [
					...
					data.users.length === 0
						? jsxRuntime.jsx("div", {
								...
								children: [
									jsxRuntime.jsx("div", { style: { fontWeight: 600 }, children: "首次启动：设置初始用户名和密码" }),
									jsxRuntime.jsx("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary,#999)", lineHeight: 1.6 }, children: "这是你创建的第一个账号（公网用户名密码登录）。请在本机设置页完成，设置后即可在公网用该账号登录。" }),
									jsxRuntime.jsx("div", { style: { display: "flex", flexDirection: "column", gap: "8px", width: "100%", maxWidth: "340px" }, children: [
										jsxRuntime.jsx("input", { id: "dsh-ia-setup-username", type: "text", placeholder: "用户名", autoComplete: "off", style: { padding: "10px 12px", borderRadius: "8px", border: "1px solid var(--dsw-alias-border-strong,rgba(128,128,128,.35))", background: "var(--dsw-alias-bg-input,#0f1115)", color: "inherit", font: "inherit", fontSize: "14px" } }),
										jsxRuntime.jsx("input", { id: "dsh-ia-setup-password", type: "password", placeholder: "密码（至少 8 个字符）", autoComplete: "new-password", style: { padding: "10px 12px", borderRadius: "8px", border: "1px solid var(--dsw-alias-border-strong,rgba(128,128,128,.35))", background: "var(--dsw-alias-bg-input,#0f1115)", color: "inherit", font: "inherit", fontSize: "14px" } }),
										jsxRuntime.jsx("input", { id: "dsh-ia-setup-confirm", type: "password", placeholder: "确认密码", autoComplete: "new-password", style: { padding: "10px 12px", borderRadius: "8px", border: "1px solid var(--dsw-alias-border-strong,rgba(128,128,128,.35))", background: "var(--dsw-alias-bg-input,#0f1115)", color: "inherit", font: "inherit", fontSize: "14px" } }),
										jsxRuntime.jsx("button", { id: "dsh-ia-setup-submit", style: Object.assign({}, btnStyle, { background: "var(--dsw-alias-accent,#3b82f6)", color: "#fff", borderColor: "transparent", padding: "8px 12px" }), onClick: setupUser, children: "设置初始账号" }),
										jsxRuntime.jsx("div", { id: "dsh-ia-setup-error", style: { fontSize: "12px", color: "var(--dsw-alias-text-danger,#e5484d)", minHeight: "14px" } }),
									]}),
								],
							})
						: jsxRuntime.jsx("div", {
								...
								children: [
									...
									jsxRuntime.jsx("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-tertiary,#aaa)" }, children: "用户名+密码登录的账号；首次启动需先设置初始账号，之后可在此添加更多用户。至少保留一个用户。" }),
								],
							}),
