			...
			var patchedFetch = function (input, init) {
				...
				if (sameOrigin(url) && shouldRewriteFetchPath(url.pathname)) {
					...
					return Promise.resolve(originalFetch.call(window, next, init)).then(function (response) {
						isTokenRequiredDenied(response).then(function (denied) {
				...
				fetch(VERIFY_URL, {
					...
					.then(function (result) {
						if (result.ok) {
							...
							if (err) err.textContent = "口令不正确，请重试";
							var b2 = document.getElementById("dsh-ia-gate-submit");
							if (b2) {
								b2.disabled = false;
								b2.textContent = "验证";
							}
					...
					.catch(function () {
						...
						var b3 = document.getElementById("dsh-ia-gate-submit");
						if (b3) {
							b3.disabled = false;
							b3.textContent = "验证";
						}
			...
			return jsxRuntime.jsx("div", {
				...
				children: [
					...
					jsxRuntime.jsx("div", {
						...
						children: "此链接暴露在公网。请输入本机「设置 → 公网访问」卡片中显示的访问口令。",
					}),
					jsxRuntime.jsx("form", {
						onSubmit: submit,
					...
					jsxRuntime.jsx("div", {
						...
						children: [
							...
							jsxRuntime.jsx("span", {
								...
								children: data.requireToken ? "已开启（非本机访问需口令）" : "未开启（完全公开，危险）",
							}),
						],
					}),
