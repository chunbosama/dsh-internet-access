 ...
 * 通道。取而代之的是一个极简登录门禁，支持两种方式：
 *
 *   - 共享访问口令（自动生成并落盘，或 profile 配置指定）；
 *   - 用户名 + 密码（scrypt 加盐哈希，首次启动需在本机设置初始用户名+密码，
 *     不再内置默认账号；也可用 config.initialUsers 预设，或经仅本机的 /users 端点管理）。
 *
 * 浏览器在非本机回环 origin 打开时，必须携带有效 cookie（口令或会话）才能使用：
 *
 *   - 一键 Cloudflare quick tunnel（cloudflared 随包分发，无账号/无域名），
 *     把 dsh web GUI 暴露到公网；
 *   - 浏览器半区把 /api、/sidebar、/git、/pet 改写为 /public 通道，本半区
 *     在门禁通过后把请求转发回 127.0.0.1（loopback 形态），从而绕过
 *     SDK 对非回环 Host 的 /api 围栏，同时保持 SDK 特权方法仅本机可达。
 *
 * 路由族（与浏览器半区契约）：
 *   GET  /api/public-access/status        仅本机回环：隧道状态/公网链接/登录方式/姿态
 ...
 *   GET/POST/DELETE /api/public-access/users   仅本机回环：用户管理（列表/增改/删）
 *   GET  /api/public-access/authorized   任意来源：门禁探测（有效 cookie → 200）
 *   POST /api/public-access/verify       任意来源：校验口令或用户名密码并签发 cookie
 *   GET/POST /public/*                   门禁通道：校验 cookie 后转发回 127.0.0.1
...
export const Config = z.object({
  ...
  /**
   * 初始用户名→密码表。首次启动（用户文件不存在或为空）时用这些用户播种；
   * 留空则不预置任何账号，首次启动需在本机设置页 / `/api/public-access/setup`
   * 创建初始用户名+密码。已存在的用户不会被覆盖。
   */
  initialUsers: z.dict(z.string()).default({}),
  /** 用户名+密码登录会话有效期（毫秒），默认 30 天。 */
  sessionTtlMs: z.number().step(1).min(60_000).default(30 * 24 * 3600 * 1000),
  /** 用户文件绝对路径（默认 $DSH_HOME/internet-access-users.json）。 */
  usersFile: z.string().default(''),
  /** 会话文件绝对路径（默认 $DSH_HOME/internet-access-sessions.json）。 */
  sessionsFile: z.string().default(''),
})
...
/** 默认用户文件路径。 */
function defaultUsersFile() {
  return join(dshHome(), 'internet-access-users.json')
}

/**
 * 用户名+密码仓库：scrypt 加盐哈希存储，文件 0600 + 原子 rename。
 * 首次启动（文件不存在/为空）时按 config.initialUsers 播种（缺省不预置任何
 * 账号，需首次在本机创建初始用户名+密码）。已存在的用户不会被覆盖。
 */
class UserStore {
  constructor(file, initialUsers, logger) {
    this.file = file
    this.initialUsers = initialUsers
    this.logger = logger
    /** @type {Record<string, { salt: string, hash: string, createdAt: string }>} */
    this.users = {}
    this.load()
    this.seed()
  }
  ...
  /** 播种 config.initialUsers 预设账号（仅当当前没有任何用户时）。 */
  seed() {
    if (Object.keys(this.users).length > 0) return
    const initial = this.initialUsers ?? {}
    for (const [username, password] of Object.entries(initial)) {
      if (typeof username === 'string' && username.trim() !== '') this.upsert(username, String(password))
    }
    if (Object.keys(this.users).length > 0) {
      this.logger?.info?.(`dsh-internet-access: 已按 config.initialUsers 播种账号 [${Object.keys(this.users).join(', ')}]`)
    }
  }

  /** 是否需要首次初始化：当前没有任何用户时返回 true。 */
  get setupRequired() {
    return Object.keys(this.users).length === 0
  }

  /** 加盐哈希（scrypt，随机 16 字节盐）。 */
  ...
  verify(username, password) {
    ...
    const candidate = scryptSync(password, record.salt, 64).toString('hex')
    return timingSafeEqual(Buffer.from(candidate, 'hex'), Buffer.from(record.hash, 'hex'))
  }
  ...
  upsert(username, password) {
    ...
    const record = UserStore.hashPassword(password)
    this.users[username] = { ...record, createdAt: this.users[username]?.createdAt ?? new Date().toISOString() }
    this.persist()
    return true
  }

  /** 删除用户，返回是否删除成功（至少保留一个用户，防止锁死）。 */
  remove(username) {
    if (!this.has(username)) return false
    if (Object.keys(this.users).length <= 1) return false
  ...
  persist() {
    try {
      mkdirSync(dirname(this.file), { recursive: true, mode: 0o700 })
      const tmp = `${this.file}.tmp-${process.pid}`
      writeFileSync(tmp, JSON.stringify({ users: this.users }, null, 2), { mode: 0o600 })
      renameSync(tmp, this.file)
    } catch (error) {
      console.warn(`[dsh-internet-access] 无法持久化用户到 ${this.file}: ${error?.message ?? error}`)
    }
  }
}

/**
 * 账号登录会话仓库：会话 id（cookie 值）→ { username, expiresAt }。
 * 内存为主，写入 $DSH_HOME/internet-access-sessions.json 以便重启后仍有效。
 */
class SessionStore {
  constructor(file, ttlMs, logger) {
    this.file = file
    this.ttlMs = ttlMs
    this.logger = logger
    /** @type {Record<string, { username: string, expiresAt: number }>} */
    this.sessions = {}
    this.load()
  }
  ...
  create(username) {
    ...
    this.sessions[id] = { username, expiresAt: Date.now() + this.ttlMs }
    this.persist()
    return id
  }
  ...
  /** 注销会话。 */
  revoke(id) {
    if (id !== undefined && this.sessions[id]) {
      delete this.sessions[id]
      this.persist()
    }
  }
  ...
  persist() {
    try {
      mkdirSync(dirname(this.file), { recursive: true, mode: 0o700 })
      const tmp = `${this.file}.tmp-${process.pid}`
      writeFileSync(tmp, JSON.stringify({ sessions: this.sessions }, null, 2), { mode: 0o600 })
      renameSync(tmp, this.file)
    } catch (error) {
      console.warn(`[dsh-internet-access] 无法持久化会话到 ${this.file}: ${error?.message ?? error}`)
    }
  }
}
...
function defaultSessionsFile() {
  return join(dshHome(), 'internet-access-sessions.json')
}
...
export function apply(ctx, config) {
  ...
  const users = new UserStore(cfg.usersFile !== '' ? cfg.usersFile : defaultUsersFile(), cfg.initialUsers, ctx.logger)
  const sessions = new SessionStore(cfg.sessionsFile !== '' ? cfg.sessionsFile : defaultSessionsFile(), cfg.sessionTtlMs, ctx.logger)
  ...
  const authorized = (req) => {
    ...
    if (safeEqual(cookie, tokens.get())) return true
    return sessions.valid(cookie)
  }

  /** 签发凭据 cookie（口令或会话）。 */
  const issueCookie = (res, value) => {
    res.writeHead(200, {
      'content-type': 'application/json; charset=utf-8',
      'set-cookie': `${cfg.cookieName}=${value}; Path=/; HttpOnly; SameSite=Lax; Max-Age=31536000`,
  ...
  // 凭据校验并签发 cookie（任意来源——公网门禁页依赖它）。
  // 支持两种登录方式：
  //   { token }                —— 访问口令
  //   { username, password }   —— 用户名 + 密码（scrypt 校验，签发会话 cookie）
  disposers.push(web.register({
    kind: 'exact',
      ...
      void (async () => {
        try {
          ...
          if (submitted !== '' && safeEqual(submitted, tokens.get())) {
            issueCookie(res, tokens.get())
            return
          }
          // 方式二：用户名 + 密码
          const username = typeof body?.username === 'string' ? body.username : ''
          const password = typeof body?.password === 'string' ? body.password : ''
          if (username !== '' && password !== '' && users.verify(username, password)) {
            const sessionId = sessions.create(username)
            issueCookie(res, sessionId)
            return
          }
          writeJson(res, 403, { ok: false, error: { code: 'bad-credentials', message: '访问口令或用户名密码不正确' } })
        } catch {
  ...
  disposers.push(web.register({
    kind: 'exact',
    path: `${CONTROL_PREFIX}/users`,
    handler: (req, res) => {
      if (!isLoopbackClient(req)) {
        req.resume()
        envelopeError(res, 403, 'invalid-request', 'forbidden', 'public-access users is loopback-only')
        return
      }
      const method = req.method ?? 'GET'
      if (method === 'GET') {
        req.resume()
        writeJson(res, 200, { ok: true, users: users.list() })
        return
      }
      if (method === 'POST') {
        void (async () => {
          try {
            ...
            if (username === '' || password === '') {
              writeJson(res, 400, { ok: false, error: { code: 'bad-request', message: 'username 和 password 必填' } })
              return
            }
            if (users.upsert(username, password)) {
              writeJson(res, 200, { ok: true, users: users.list() })
            } else {
              writeJson(res, 400, { ok: false, error: { code: 'bad-request', message: '无效的用户名或密码' } })
            }
          } catch {
            writeJson(res, 400, { ok: false, error: { code: 'bad-request', message: '无效请求' } })
          }
      ...
      if (method === 'DELETE') {
        void (async () => {
          try {
            ...
            const username = typeof body?.username === 'string' ? body.username : ''
            if (username === '') {
              writeJson(res, 400, { ok: false, error: { code: 'bad-request', message: 'username 必填' } })
              return
            }
            if (users.remove(username)) {
              writeJson(res, 200, { ok: true, users: users.list() })
            } else {
              writeJson(res, 400, { ok: false, error: { code: 'bad-request', message: '用户不存在或不能删除最后一个用户' } })
            }
          } catch {
            writeJson(res, 400, { ok: false, error: { code: 'bad-request', message: '无效请求' } })
          }
  ...
  // 首次初始化（仅回环、且仅当尚无任何用户时）：创建初始用户名+密码账号。
  // 已有任何用户后返回 409，防止公网抢占首次账号。
  disposers.push(web.register({
    kind: 'exact',
    path: `${CONTROL_PREFIX}/setup`,
    handler: (req, res) => {
      if (!isLoopbackClient(req)) {
        req.resume()
        envelopeError(res, 403, 'invalid-request', 'forbidden', 'public-access setup is loopback-only')
        return
      }
      void (async () => {
        try {
          if (!users.setupRequired) {
            req.resume()
            writeJson(res, 409, { ok: false, error: { code: 'already-initialized', message: '初始账号已创建，请使用登录入口' } })
            return
          }
          const body = await readBoundedJson(req, 4096)
          const username = typeof body?.username === 'string' ? body.username.trim() : ''
          ...
          if (password !== confirm) {
            writeJson(res, 400, { ok: false, error: { code: 'password-mismatch', message: '两次输入的密码不一致' } })
            return
          }
          if (password.length < 8) {
            writeJson(res, 400, { ok: false, error: { code: 'weak-password', message: '密码至少 8 个字符' } })
            return
          }
          if (users.upsert(username, password)) {
            writeJson(res, 200, { ok: true, user: username, users: users.list() })
          } else {
            writeJson(res, 400, { ok: false, error: { code: 'bad-request', message: '无效的用户名或密码' } })
          }
        } catch {
