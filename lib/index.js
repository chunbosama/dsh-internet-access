 ...
 *   - 用户名 + 密码（scrypt 加盐哈希，首次启动播种初始用户 admin/100410zzr，
 *     可通过 config.initialUsers 覆盖，或经仅本机的 /users 端点管理）。
 *
...
export const Config = z.object({
   ...
   * 留空则自动创建内置初始用户 `admin / 100410zzr`。已存在的用户不会被覆盖。
   */
...
/** 内置初始用户（config.initialUsers 留空时使用）。 */
const BUILTIN_INITIAL_USER = { admin: '100410zzr' }

/**
 * 用户名+密码仓库：scrypt 加盐哈希存储，文件 0600 + 原子 rename。
 * 首次启动（文件不存在/为空）时播种初始用户：优先 config.initialUsers，
 * 否则内置 admin / 100410zzr。
 */
class UserStore {
  ...
  seed() {
    ...
    const initial = Object.keys(this.initialUsers).length > 0 ? this.initialUsers : BUILTIN_INITIAL_USER
    for (const [username, password] of Object.entries(initial)) {
      this.upsert(username, String(password))
    }
    if (Object.keys(this.users).length > 0) {
      this.logger?.info?.(`dsh-internet-access: 已播种初始用户 [${Object.keys(this.users).join(', ')}]`)
    }
  }
